...
## At a Glance
  ...
  - If you want to add custom overlays or base layers, you'll need to host them using a piece of software called [Geoserver][geoserver].
...
Building on the excellent foundation provided by Omeka, Neatline is a plugin (just like plugins for Wordpress or modules for Drupal) that grafts new functionality onto the core Omeka feature set. Neatline adds an interactive map-making environment that makes it possible to create Neatline _exhibits_, each of which is populated with its own collection of _records_, which can be synchronized with Dublin Core records in the underlying Omeka collection.

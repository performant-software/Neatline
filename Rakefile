...
namespace :test do
  ...
  task :all do
    ...
    sh %{cd views/shared/javascripts/tests/suites/map && rake jasmine:ci}
    sh %{cd views/shared/javascripts/tests/suites/editor && rake jasmine:ci}
  end
  ...
  desc 'Boot the map Jasmine suite'
  task :map do
    Rake::Task['test:server'].invoke

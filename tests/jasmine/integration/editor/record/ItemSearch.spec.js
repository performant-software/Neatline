...
describe('Record | Item Search', function() {
  ...
  beforeEach(function() {
    ...
    el = {
      autocomplete: $(NL.vw.TEXT.ui.item.autocomplete('widget')[0])
    };
  ...
  it('should autocomplete Omeka items under search box', function() {
    ...
    NL.vw.TEXT.ui.item.autocomplete('search', 'item');
  ...
  it('should populate id and title on item select', function() {
    ...
    // Should populate id.
    expect(NL.vw.TEXT.ui.item).toHaveValue(id);
    expect(NL.vw.RECORD.model.get('item_id')).toEqual(id);

    // Should populate title.
    expect(NL.vw.TEXT.ui.title).toHaveValue('Item 3');
